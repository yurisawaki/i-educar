...
return new class extends clsListagem {
    ...
    {
        $this->titulo = __('Empresas');

        $this->addCabecalhos(coluna: [__('Razão Social'), __('Nome Fantasia')]);

        $this->campoTexto(nome: 'fantasia', campo: __('Nome Fantasia'), valor: $_GET['fantasia'], tamanhovisivel: '50', tamanhomaximo: '255');
        $this->campoTexto(nome: 'razao_social', campo: __('Razão Social'), valor: $_GET['razao_social'], tamanhovisivel: '50', tamanhomaximo: '255');
        $this->campoCnpj(nome: 'id_federal', campo: __('CNPJ'), valor: $_GET['id_federal']);
        ...
        if ($empresas) {
            foreach ($empresas as $empresa) {
                ...
                $this->addLinhas(linha: [
                    "<a href='empresas_det.php?cod_empresa={$cod_empresa}'><img src='imagens/noticia.jpg' border=0>{$razao_social}</a>",
                    "<a href='empresas_det.php?cod_empresa={$cod_empresa}'>{$nome_fantasia}</a>"
                ]);
            }
        }
        // Paginador
        ...
        $this->breadcrumb(currentPage: __('Listagem de pessoas jurídicas'), breadcrumbs: [
